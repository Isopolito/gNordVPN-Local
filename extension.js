            ...
            _refresh() {
                ...
                this._vpn.getStatus()
                    .then(status => {
                        ...
                        // Update the timeout based on the current state
                        timeoutInSec = this.settings.get_boolean(`extra-large-button`) ? 1 : status.currentState.refreshTimeout;
                    ...
                    .catch(error => {
                        // Handle any errors that may occur while getting the VPN status
                        logError(error, "Gnordvpn: Failed to refresh VPN status");
                    })
                    .finally(() => {
                        this.isRefreshing = false;
                        this._unlockUI();
